...
export const ConsultantDashboard: React.FC = () => {
  ...
  const mentalHealthForms = useMemo(() => db.getMentalHealthForms(), []);
  ...
  const mentalHealthConcerns = useMemo(() => {
    return studentsWithAnalysis.filter(item => {
      const latestForm = item.forms.sort(
        (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      )[0];
      
      if (!latestForm) return false;
      
      return latestForm.stressLevel > 7 || 
             latestForm.anxietyLevel > 7 ||
             latestForm.motivationLevel < 4;
    });
  }, [studentsWithAnalysis]);
  ...
  const mentalHealthTrends = useMemo(() => {
    ...
    mentalHealthForms.forEach(form => {
      ...
      if (!trends[month]) {
        trends[month] = { stress: 0, anxiety: 0, motivation: 0, count: 0 };
      }
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-dark-100">سلام، {consultant.fullName}</h1>
          <p className="text-dark-400 mt-1">پنل مشاوره</p>
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-dark-400 text-sm">
                {highRiskStudents.filter(s => s.analysis.riskScore > 80).length} دانش‌آموز در وضعیت بحرانی هستند و نیاز به توجه فوری دارند
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader
            title="توزیع ریسک"
            subtitle="وضعیت کلی دانش‌آموزان"
          ...
          <div className="h-64">
            {mentalHealthTrends.length > 0 ? (
              ...
              <div className="h-full flex items-center justify-center">
                <p className="text-dark-500">داده‌ای موجود نیست</p>
              </div>
            )}
          </div>
        </Card>
      ...
      <Card>
        <CardHeader
          ...
          subtitle="بر اساس امتیاز ریسک"
          icon={<AlertTriangle className="w-5 h-5" />}
          action={<Badge variant="danger">{highRiskStudents.length} نفر</Badge>}
        />
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-left">
                      <p className="text-dark-300 text-sm">{mainIssue?.category}</p>
                      <p className="text-dark-500 text-xs">{mainIssue?.description}</p>
                    </div>
                    <Badge
                      variant={analysis.riskScore > 80 ? 'danger' : 'warning'}
                    >
                      {analysis.riskScore > 80 ? 'بحرانی' : 'پرخطر'}
                    </Badge>
                  </div>
                </div>
              );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card hover className="cursor-pointer">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-medium text-dark-100">ثبت جلسه مشاوره</h3>
              <p className="text-dark-500 text-sm">ثبت گزارش جلسه جدید</p>
            </div>
          </div>
        </Card>
        ...
        <Card hover className="cursor-pointer">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-medium text-dark-100">جلسات پیش‌رو</h3>
              <p className="text-dark-500 text-sm">برنامه‌ریزی جلسات</p>
            </div>
          </div>
        </Card>
        ...
        <Card hover className="cursor-pointer">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-medium text-dark-100">گزارش پیشرفت</h3>
              <p className="text-dark-500 text-sm">مشاهده روند بهبود</p>
            </div>
          </div>
        </Card>
      </div>
